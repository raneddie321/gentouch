...
export default function BookingSection() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', email: '', service: '', date: '', time: '', notes: '', homeVisit: false });
      ...
      <section id="booking" dir="rtl" className="py-24 px-4 bg-background">
        <div className="max-w-xl mx-auto text-center">
          <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', duration: 0.7 }}>
            ...
            <h3 className="font-playfair text-3xl font-bold text-foreground mb-4">הזמנה התקבלה</h3>
            ...
            <p className="font-heebo text-muted-foreground/70 text-sm mb-8">הזמנה התקבלה, נחזור אליך לאישור בהקדם.</p>
            <button
              onClick={() => { setSuccess(false); setForm({ name: '', phone: '', email: '', service: '', date: '', time: '', notes: '', homeVisit: false }); }}
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <div>
            ...
            <label className="mt-3 flex items-start gap-3 text-primary bg-primary/5 rounded-xl px-4 py-3 cursor-pointer border border-primary/15">
              <input
                type="checkbox"
              ...
              />
                ...
                <span className="font-heebo text-sm">טיפול בבית הלקוח באזור {HOME_VISIT_AREA}: תוספת {HOME_VISIT_FEE} למחיר הטיפול.</span>
            </label>
          </div>
          ...
          {/* Notes */}
          <div>
            <h3 className="font-heebo text-sm font-semibold text-foreground/60 uppercase tracking-widest mb-4">בקשות מיוחדות למטפל</h3>
            <label className="font-heebo text-sm font-medium text-foreground/80 mb-2 flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-primary" /> בקשות מיוחדות למטפל
            </label>
            <Textarea
              value={form.notes}
            ...
            />
            <p className="font-heebo text-xs text-muted-foreground/60 mt-2">{form.notes.length}/{NOTES_MAX_LENGTH} תווים. המידע יישמר בסוד ויועבר ישירות למטפל לפני הטיפול</p>
          </div>
          ...
          {(selectedService || form.date || form.time) && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-primary/5 border border-primary/20 rounded-2xl px-5 py-4 space-y-1">
              ...
              {selectedService && <p className="font-heebo text-sm text-foreground">טיפול בבית הלקוח: <strong>{form.homeVisit ? `כן, תוספת ${HOME_VISIT_FEE} באזור ${HOME_VISIT_AREA}` : 'לא'}</strong></p>}
              ...
              {form.name && <p className="font-heebo text-sm text-foreground">שם: <strong>{form.name}</strong></p>}
              {form.phone && <p className="font-heebo text-sm text-foreground">טלפון: <strong>{form.phone}</strong></p>}
              {form.email && <p className="font-heebo text-sm text-foreground">אימייל: <strong>{form.email}</strong></p>}
              <p className="font-heebo text-sm text-foreground">בקשות מיוחדות: <strong>{form.notes || 'לא צוינו'}</strong></p>
            </motion.div>
          )}
