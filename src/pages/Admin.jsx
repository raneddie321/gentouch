...
import { DEFAULT_SERVICE_SETTINGS, getServiceSettings, HOME_VISIT_AREA, HOME_VISIT_FEE, resetServiceSettings, saveServiceSettings } from '@/lib/serviceSettings';
import { formatIsraelDateTime, getAppointments, getDayAvailability, getIsraelTimestamp, saveDayAvailability, TIME_SLOTS, updateAppointmentStatus } from '@/lib/appointments';

const ADMIN_CODE = 'poiuytrewq1';
const today = getIsraelTimestamp().date;
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-background font-heebo">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <section className="bg-primary/5 border border-primary/20 rounded-2xl p-5 md:p-6 mb-8">
          <h2 className="font-heebo text-xl font-bold text-foreground mb-3">מדריך קצר לממשק הניהול</h2>
          <div className="grid md:grid-cols-2 gap-3 text-sm text-foreground/75">
            <p><strong>זמינות יומית:</strong> בחרו תאריך וסמנו אילו שעות פתוחות להזמנה. שעה שכבר הוזמנה נחסמת אוטומטית.</p>
            <p><strong>שירותים ומחירים:</strong> עדכנו שם טיפול, מחיר, משך, תמונה ותיאור. לאחר שינוי לחצו “שמור”.</p>
            <p><strong>תורים:</strong> סננו לפי סטטוס, אשרו/בטלו הזמנה, וחזרו ל“ממתין” במקרה הצורך.</p>
            <p><strong>עד הבית:</strong> הטופס מוסיף אפשרות עד הבית לכל טיפול באזור {HOME_VISIT_AREA}, בתוספת {HOME_VISIT_FEE}.</p>
          </div>
        </section>
              ...
              <div key={appointment.id} className="bg-card border border-border/50 rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
                <div className="p-5 md:p-6">
                  ...
                  <div className="flex flex-wrap gap-3 mt-4">
                    ...
                    {(appointment.servicePrice || appointment.serviceDuration) && (
                      <span className="font-heebo text-xs px-3 py-1.5 rounded-lg font-medium bg-muted text-muted-foreground">
                        {appointment.servicePrice} {appointment.serviceDuration && `| ${appointment.serviceDuration}`}
                      </span>
                    )}
                    {appointment.homeVisit && (
                      <span className="font-heebo text-xs px-3 py-1.5 rounded-lg font-medium bg-primary/10 text-primary">
                        עד הבית: תוספת {appointment.homeVisitFee || HOME_VISIT_FEE}
                      </span>
                    )}
                    {appointment.date && (
                    ...
                    {(appointment.order_received_display || appointment.created_date) && (
                      <span className="flex items-center gap-1.5 font-heebo text-xs text-muted-foreground bg-muted/50 px-3 py-1.5 rounded-lg">
                        התקבלה: {appointment.order_received_display || formatIsraelDateTime(appointment.created_date)}
                      </span>
                    )}
                  </div>
                  ...
                  {appointment.notes && (
                    <div className="mt-3 p-3 bg-muted/40 rounded-xl">
                      <p className="font-heebo text-sm text-muted-foreground flex items-start gap-2">
                        ...
                        <span className="font-medium text-foreground/80">בקשות מיוחדות:</span>
                        {appointment.notes}
                      </p>
                    </div>
                  )}
